...
def write_hpp(outfile_name, ai):
    ...
    if isinstance(ai, list):
        ...
        ai_file.write('''
inline
uint32_t* get_data(int id, int key, boost::circular_buffer<uint8_t> & data, uint32_t* buffer)
{
  int idx=0;
    ...
    )
{
  ...
  double ret;
  uint32_t buffer[512];
  ret=ai[bbid][bb_key]->predict(get_data(bbid, bb_key, hamming_distance, buffer));
  if(fout!=nullptr)
    *fout<<cycles<<","<<ret<<"\\n";
  return;
}
